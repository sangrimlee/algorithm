...
import { getIntersectionNode } from './0160';
...
describe('LeetCode 0160', () => {
  ...
  test('Example 1', () => {
    const { headA, headB, headC } = createExample(8, [4, 1, 8, 4, 5], [5, 6, 1, 8, 4, 5], 2, 3);
    expect(getIntersectionNode(headA, headB)).toEqual(headC);
  });

  test('Example 2', () => {
    const { headA, headB, headC } = createExample(2, [1, 9, 1, 2, 4], [3, 2, 4], 3, 1);
    expect(getIntersectionNode(headA, headB)).toEqual(headC);
  });
