...
export function shortestPath(grid: number[][], k: number): number {
      ...
      for (const [dy, dx] of DIRECTIONS) {
        ...
        if (0 <= ny && ny < n && 0 <= nx && nx < m) {
          ...
          if (grid[ny][nx] === 0) {
            ...
            if (
              0 < remain &&
              costs[y][x][remain] + 1 < costs[ny][nx][remain - 1]
            ) {
  ...
  const answer = costs[n - 1][m - 1].reduce(
...
function create3DArr<T>(
  ...
  return new Array(n)
    .fill(undefined)
    .map(() =>
      new Array(m).fill(undefined).map(() => new Array(k).fill(initValue))
    );
}
